...
def estimate_rigid_transform(
    ...
    """Estimate rigid transform that maps source points to target points."""
    ...
    left_svd, _, right_svd_t = np.linalg.svd(covariance)
    rotation = right_svd_t.T @ left_svd.T
...
def apply_rigid_transform_to_world_points(
    ...
    for view_index in range(transformed.shape[0]):
        ...
        flat_points = (flat_points @ rotation.T) + translation
        transformed[view_index] = flat_points.reshape(transformed[view_index].shape)
    return transformed


def apply_rigid_transform_to_extrinsics(
    extrinsics: np.ndarray, rotation: np.ndarray, translation: np.ndarray
) -> np.ndarray:
    ...
    for view_index in range(aligned_extrinsics.shape[0]):
        ...
        aligned_extrinsics[view_index, :3, :3] = cam_rotation @ rotation.T
        aligned_extrinsics[view_index, :3, 3] = (
            cam_translation - cam_rotation @ rotation.T @ translation
        )
...
def prepare_side_view_bundle(
    ...
    vggt_info = load_vggt_info(vggt_path)
    sam_outputs = load_sam_outputs(sam_path)
...
def prepare_frame_data(
    ...
    rotation, translation, transform = estimate_rigid_transform(
        source_points=right_bundle.centered_kpts_3d,
        target_points=left_bundle.centered_kpts_3d,
    )
    aligned_right_world_points = apply_rigid_transform_to_world_points(
        right_bundle.centered_world_points, rotation, translation
    )
    aligned_right_kpts = (right_bundle.centered_kpts_3d @ rotation.T) + translation
    aligned_right_extrinsics = apply_rigid_transform_to_extrinsics(
        right_bundle.centered_extrinsics, rotation, translation
    )
...
def analyze_run(
    ...
    frame_indices = list_available_frame_indices(run_name, data_root)
